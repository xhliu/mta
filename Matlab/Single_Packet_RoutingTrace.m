...
%   from the source. Mum_Routes records the number of routes to deliver
...
% srcDir = 'D:\Q''s files\Research\Notes\My Dropbox\tOR\Qiao\Job';
% srcDir2 = '134';   %Defined by users
% srcDir3 = 'raw data\';
% DirDelimiter='/';
...
for Sender_Index = 1:size(Node_Table, 1)
    ...
    for Receiver_Index = 1:size(Node_Table, 1)
        ...
        if Sender ~= Receiver
            ...
            Num_Recv = size(find(Unique_Packet_Log(:, 1) == RX & Unique_Packet_Log(:, LastHopSender) == Sender), 1);
            if Num_Send ~= 0
                if Num_Recv ~=0
