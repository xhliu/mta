...
//to compute MTE error
command error_t RoutingTable.getPathDelay(uint32_t *delay_mean, uint32_t *delay_var) {
	uint8_t i, queue_size;
	uint8_t pending;
	routing_table_entry *ne;
	...
	for (i = 0; i < routingTableActive; i++) {
		...
		//consider the path thru parent
		if (ne->neighbor == call Utils.getParent()) {
			queue_size = call DataPanel.queueSize(&pending);
			//link delay + node delay
			*delay_mean = (queue_size + 1) * ne->out_mac_delay_mean + ne->node_delay_etxs[0].node_delay_mean;
			*delay_var = (queue_size + 1) * ne->out_mac_delay_var + ne->node_delay_etxs[0].node_delay_var;
	    	call UartLog.logEntry(DBG_FLAG, DBG_ESTIMATE_ERR_2_FLAG, queue_size, *delay_mean);
	    	dbg("TreeRouting", "%s queue size <%u, %u>\n", __FUNCTION__, queue_size, pending);
			return SUCCESS;
		}
	}
	return FAIL;
...
*/
    ...
    queue_size = call DataPanel.queueSize(&pending);
	//if (is_forwarder_called && queue_size > 0)
		//queue_size--;
    //TODO: hybrid: sender-based and receiver-based pkt time estimation
    call DataPanel.getTxPktTime(&weighted_mac_delay_mean, &weighted_mac_delay_var);
    ...
    if (is_forwarder_called) {
    	if (0 == queue_size)
    		dbg("TreeRoutingDbg", "%s: err 1\n", __FUNCTION__);
    } else {
    	if (queue_size > 0)
    		dbg("TreeRoutingDbg", "%s: err 2\n", __FUNCTION__);    	
    }
...
command uint8_t RoutingTable.getParents(uint32_t relative_deadline, uint16_t e2e_pdr_req, am_addr_t parents[], uint16_t parent_e2e_pdr_reqs[]) {
	...
	for (i = 0; i < routingTableActive; i++) {
		 ...
		 */
		if (24 == TOS_NODE_ID)
		 	dbg("TreeRouting", "neighbor %hu: link delay %u\n", ne->neighbor, ne->out_link_delay_mean);
#ifdef MCMP
		//hop count DAG
